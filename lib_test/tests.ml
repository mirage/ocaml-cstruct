...
let of_string_as_sexp str = Cstruct_sexp.t_of_sexp (Sexplib.Sexp.of_string str)
...
let sexp_repr =
  ...
  let cs2 = shift cs1 2 and cs3 = sub cs1 2 4 in
...
let sexp_writer () =
  sexp_repr
  |> List.iter @@ fun (cs, str) ->
     assert_string_equal str (to_string_as_sexp cs)

let sexp_reader () =
  sexp_repr
  |> List.iter @@ fun (cs, str) -> assert_cs_equal cs (of_string_as_sexp str)
...
let concat_ex =
  let open Cstruct in
  List.map
    (fun (ss, s) -> (List.map of_string ss, of_string s))
    [ ([], "");
      ([ "abcd" ], "abcd");
      ([ "" ], "");
      ([ ""; "" ], "");
...
let concat_samples () =
  concat_ex
  |> List.iter @@ fun (css, cs) -> assert_cs_equal cs (Cstruct.concat css)
...
let concat_random ~n () =
  let rec explode cs =
    ...
    if n = 0 then []
      ...
      Cstruct.sub cs 0 k :: explode (Cstruct.shift cs k)
  in
  for _i = 1 to n do
    let cs = random_cs () in
...
let fillv () =
  let test src buf_size =
    ...
    assert (Cstruct.equal whole (Cstruct.concat src))
  in
  test [] 0;
  test [] 16;
  ...
  test
    [ Cstruct.of_string "abc"; Cstruct.of_string ""; Cstruct.of_string "def" ]
    0;
  test
    [ Cstruct.of_string "abc"; Cstruct.of_string ""; Cstruct.of_string "def" ]
    3;
...
let check_alignment_large () =
  ...
  if Sys.word_size > 32 then
    let msg =
      Fmt.strf "alignement large: int-size:%d len=%d" Sys.word_size
        (Int64.to_int 4294967296L)
    in
    Alcotest.(check bool) msg (check ()) false
  else
    try
      let _ = check () in
      Alcotest.fail "alignement should raise"
    with Invalid_argument _ -> ()

let rev_empty () = assert_cs_equal Cstruct.empty (Cstruct.rev Cstruct.empty)
...
let hexdump_empty () = test_hexdump Cstruct.empty ""

let hexdump_small () = test_hexdump (Cstruct.of_hex "00010203") "00 01 02 03"

let hex_multiline = Cstruct.of_hex "000102030405060708090a0b0c0d0e0f101112"

let hexdump_multiline () =
  test_hexdump hex_multiline
...
let hexdump_aligned_to_half () =
  test_hexdump (Cstruct.of_hex "0001020304050607") "00 01 02 03 04 05 06 07"

let hexdump_in_box () =
  test_hexdump ~format:"This is a box : %a" hex_multiline
    ( "This is a box : 00 01 02 03 04 05 06 07  08 09 0a 0b 0c 0d 0e 0f\n"
    ^ "                10 11 12" )

let suite =
  [ ("fillv", [ ("fillv", `Quick, fillv) ]);
    ( "sexp",
    ...
    ("append", [ ("append is concat", `Quick, append_is_concat ~n:5000) ]);
    ( "alignment",
