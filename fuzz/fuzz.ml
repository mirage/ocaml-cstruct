...
let cstruct =
  choose [ map [ int8 ] create; map [ range 0x10000; int; int ] create_sub ]
...
let () =
  ...
  (* Prevent accidentally running in quickcheck mode *)
  add_test ~name:"blit" [ cstruct; int; cstruct; int; int ]
    (fun src srcoff dst dstoff len ->
      try Cstruct.blit src srcoff dst dstoff len
      with Invalid_argument _ ->
        check
          ( srcoff < 0
          || srcoff > Cstruct.len src
          || dstoff < 0
          || dstoff > Cstruct.len src
          || len < 0
  ...
  add_test ~name:"sexp" [ buffer ] (fun b ->
      ...
      |> check_eq
           ~cmp:(fun x y ->
             Cstruct.compare (Cstruct.of_bigarray x) (Cstruct.of_bigarray y) )
           b );
  add_test ~name:"of_bigarray" [ buffer; option int; option int ]
    (fun b off len ->
  ...
  add_test ~name:"shift" [ cstruct; int ] (fun base off ->
      ...
      | exception Invalid_argument _ ->
          check (off < 0 || off > Cstruct.len base) );
  add_test ~name:"copy" [ cstruct; int; int ] (fun base off len ->
      match Cstruct.copy base off len with
      | x ->
          check (String.length x = len);
          check
            (String.equal x (Cstruct.sub base off len |> Cstruct.to_string))
      | exception Invalid_argument _ ->
          check
            ( off < 0 || len < 0
            || off + len < 0
            || off + len > Cstruct.len base ) );
  add_test ~name:"blit_from_bytes" [ bytes; int; cstruct; int; int ]
    (fun src srcoff dst dstoff len ->
      match Cstruct.blit_from_bytes src srcoff dst dstoff len with
      | () ->
          check
            (Cstruct.equal
               (Cstruct.sub (Cstruct.of_bytes src) srcoff len)
               (Cstruct.sub dst dstoff len))
      | exception Invalid_argument _ ->
          check
            ( srcoff < 0
            || srcoff > Bytes.length src
            || dstoff < 0
            || dstoff > Bytes.length src
            || len < 0
            || len > Bytes.length src - srcoff
            || len > Cstruct.len dst - dstoff ) );
  add_test ~name:"blit_to_bytes" [ cstruct; int; bytes; int; int ]
    (fun src srcoff dst dstoff len ->
      match Cstruct.blit_to_bytes src srcoff dst dstoff len with
      | () ->
          check
            (Cstruct.equal
               (Cstruct.sub src srcoff len)
  ...
  add_test ~name:"set_len" [ cstruct; int ] (fun base len ->
      ...
      | x ->
          (* check_within ~base x; *)
          (* Maybe deprecate set_len extending the allocation? *)
  ...
  add_test ~name:"BE.set_uint64" [ cstruct; int ] (fun c off ->
      ...
      | () -> check in_range
      | exception Invalid_argument _ -> check (not in_range) );
  add_test ~name:"lenv" [ list cstruct ] (fun cs -> check (Cstruct.lenv cs >= 0));
  add_test ~name:"copyv" [ list cstruct ] (fun cs ->
      check
        (String.equal (Cstruct.copyv cs)
           (Cstruct.concat cs |> Cstruct.to_string)) );
